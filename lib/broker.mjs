...
// 又不能小到把「扩展正忙、马上会接手」误判成丢件——8 秒是实测比较稳的值。
...
export function createBroker({
  ...
  function checkDeliveryAcks() {
    for (const task of tasks.values()) {
      if (task.done || task.state !== 'sent' || !task.awaitingAckSince) continue;
      if (now() - task.awaitingAckSince < ackTimeoutMs) continue;
      task.awaitingAckSince = null;
      if (task.attempts >= maxDeliveries) {
        finishTask(task, {
          ok: false,
          error: `连续 ${task.attempts} 次交付都没有得到页面响应（扩展或标签页可能不稳定，或浏览器把它的服务工作线程杀了）。`,
          code: 'WEB_NO_ACK',
        });
        continue;
      }
      const lastPhase = task.phases.slice(-1)[0] || '（还没有阶段）';
      task.phases.push(`第 ${task.attempts} 次交付没有回应，正在重新派发（最后阶段：${lastPhase}）`);
      event('task_redeliver', { taskId: task.id, attempt: task.attempts, lastPhase });
      for (const p of task.progressWriters) { try { p.write(JSON.stringify({ type: 'progress', phase: `第 ${task.attempts} 次交付没有回应，正在重新派发` }) + '\n'); } catch { /* ignore */ } }
      try { task.onProgress?.(`第 ${task.attempts} 次交付没有回应，正在重新派发`); } catch { /* ignore */ }
  ...
  function dispatchNext() {
    ...
    // 交付确认（ack）：扩展拿到任务后必须尽快给第一条 progress。
    // 为什么要它：把任务写进一个**已经没人读的响应**（浏览器杀掉 MV3 服务工作线程、连接被中断、
    // 页面正在重载）时，服务端看起来"派发成功"，实际任务凭空消失——实测就是这种 120 秒静默。
    ...
    sendJson(poller.res, { task: { id, lease: task.lease, prompt: task.prompt, timeoutMs: task.timeoutMs || timeoutMs }, activeTaskId: id });
  }
  ...
  function createTask({ prompt, timeoutMs: taskTimeout, streamRes, onProgress = null, onFinish = null }) {
  ...
  async function handle(req, res) {
    ...
    if (req.method === 'POST' && (path === '/ext/progress' || path === '/ext/result')) {
      ...
      task.awaitingAckSince = null;   // 收到任何回报都算「这次交付落到了活人手里」，撤销重投计时
  ...
  return {
     ...
     */
    run({ prompt, timeoutMs: budget, signal, onProgress = () => {} }) {
      return new Promise((resolve, reject) => {
