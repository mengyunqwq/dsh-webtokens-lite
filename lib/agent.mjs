...
async function poll(baseUrl, token, deviceName) {
  const res = await fetch(baseUrl + '/agent/runner/poll', {
    ...
    body: JSON.stringify({ holdMs: POLL_HOLD_MS, name: deviceName, platform: platformTag(), version: AGENT_VERSION, capabilities: ['web_prompt'] }),
  });
...
/**
 * 回报结果。**永不抛错**：它是"尽力而为"的上报，中转站自己也有超时兜底。
 * 为什么必须这样：调用点是 `await submit(...)`，其中失败上报那一次位于 catch 块内部——
 * 一旦它抛出，异常会越过循环边界让 runAgent 整体 reject，连接器进程退出（设备离线，
 * 只能等下次开机自启才回来），而当时正在跑的任务也一起丢掉。一次网络抖动就够触发。
 */
async function submit(baseUrl, token, payload) {
  try {
    ...
    if (!res.ok) log(`回报结果失败 HTTP ${res.status}（不影响本机继续取活）`);
  } catch (error) {
    log(`回报结果出错（网络）：${error.message}（不影响本机继续取活）`);
  }
}
...
export async function runAgent({ server, token, deviceName, localToken, localPort = 3081, timeoutMs = DEFAULT_TIMEOUT_MS, enableTools = true }) {
  ...
  for (;;) {
    ...
    try {
      ...
      const { reply, elapsedMs, warnings, attempts, phases } = await callBridge({
      ...
      log(`任务完成 ${job.jobId}：${reply.kind}${reply.kind === 'tool_calls' ? `（${reply.calls.length} 个工具调用）` : ''}，${elapsedMs}ms，尝试 ${attempts} 次`);
      await submit(baseUrl, token, {
        jobId: job.jobId, ok: true, output: JSON.stringify(completion), error: '',
        // phases 一并回传：中转站可以把它显示给调用方，卡在哪一步一目了然
        meta: { agent: AGENT_VERSION, kind: reply.kind, elapsedMs, attempts, phases, warnings: warnings?.length ? warnings : undefined },
      });
    } catch (error) {
      log(`任务失败 ${job.jobId}：${error.code || ''} ${error.message}`);
      await submit(baseUrl, token, {
        jobId: job.jobId, ok: false, error: String(error.message || error).slice(0, 900),
        meta: { agent: AGENT_VERSION, code: error.code || null, phases: error.phases },
      });
    }
  }
}
