...
import { PLUGIN_DIR, DEFAULT_TIMEOUT_MS, STALL_MS, SESSION_MODE, SESSION_NAME, SESSION_BUDGET, FORMAT_GUARD } from './config.mjs';
...
export function retryable(error) {
  ...
  if (['WEB_TIMEOUT', 'WEB_TRANSPORT_ERROR', 'WEB_ABORTED', 'WEB_PAGE_ERROR', 'WEB_DISCONNECTED',
    'WEB_TOOL_ARGUMENTS', 'WEB_TOOL_UNKNOWN', 'WEB_REPLY_JSON', 'WEB_REPLY_KIND', 'WEB_REPLY_TEXT',
...
function submitWithWatchdog({ token, port, timeoutMs, request, signal, onProgress, phases }) {
  return new Promise((resolve, reject) => {
    ...
    submitRemote({ port, token, timeoutMs }, request, ac.signal, (p) => {
      ...
      onProgress?.(p);
    }).then(settle(resolve), settle((error) => {
      if (stalled) {
